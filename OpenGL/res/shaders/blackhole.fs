...
vec4 background(vec3 ray)
{
    ...
    float brightness = value(uv*3.0, 100.0); //(poor quality) "stars" created from value noise
    float color = value( uv*2.0, 20.0); 
    brightness = pow(abs(brightness), 256.0);
...
void main()
{
    ...
    for(int j=0;j<AA;++j)
    {
        for(int i=0;i<AA;++i)
        {
            ...
            angle.y = (2.0*iMouse.y/iResolution.y)*3.14 + 0.1 + 3.14;
            float dist = length(pos);
            Rotate(pos,angle);
            angle.xy -= min(0.3/dist , 3.14) * vec2(1, 0.5);
            Rotate(ray,angle);
            ...
            for(int disks = 0; disks<20; ++disks) //steps
            {
                for (int h = 0; h < 6; ++h) //reduces tests for exit conditions (to minimise branching)
                {
                    ...
                    float closeLimit = centDist*0.1 + 0.05*centDist*centDist*(1.0/_Size); //limit step size close to BH
                    ...
                    glow += vec4(1.2, 1.1, 1.0, 1.0) *(0.01*stepDist * invDistSqr * invDistSqr * clamp(centDist*(2.0) - 1.2, 0.0, 1.0)); //adds fairly cheap glow
                }
            ...
            col = outCol;
            col.rgb = pow(abs(col.rgb), vec3(0.6));
